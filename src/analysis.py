...
def degree_stratified_accuracy(gcn_pred, gat_pred, data, output_dir="results"):
    """Bin test nodes by citation count and compare model accuracy per bin."""
    ...
    for lo, hi, label in bins:
        ...
        if mask.sum() == 0:
            continue
        idx = test_idx[mask]
        gcn_accs.append((gcn_p[idx] == y_true[idx]).mean())
        gat_accs.append((gat_p[idx] == y_true[idx]).mean())
    ...
    plt.tight_layout()
    plt.savefig(f"{output_dir}/degree_vs_accuracy.png", dpi=150, bbox_inches="tight")
    plt.close()
...
def disagreement_analysis(gcn_pred, gat_pred, data, output_dir="results"):
    """Identify test nodes where GCN and GAT disagree and analyze patterns."""
    ...
    plt.tight_layout()
    plt.savefig(f"{output_dir}/disagreement_analysis.png", dpi=150, bbox_inches="tight")
    plt.close()


def attention_analysis(gat_model, data, output_dir="results"):
    """Extract and analyze GAT layer-1 attention weights."""
...
def embedding_comparison(gcn_model, gat_model, data, output_dir="results"):
    """Side-by-side t-SNE of GCN and GAT hidden-layer representations."""
    gcn_model.eval()
    gat_model.eval()
    with torch.no_grad():
    ...
    plt.tight_layout()
    plt.savefig(f"{output_dir}/embedding_comparison.png", dpi=150, bbox_inches="tight")
    plt.close()
